...
// MARK: - Request Options Modifiers
    ...
    /// - Parameter bundle: Bundle in which mock file is located. Defaults to main bundle
    /// - Parameter ofType: The extension of the file. Defaults to `.json` if not provided.
    /// - Returns: NetworkResource
    @objc @discardableResult func mock(withFile: String, bundle: Bundle = Bundle.main, ofType: String = "json") -> NetworkResource {
        guard let path = bundle.path(forResource: withFile, ofType: ofType), let data = try? Data(contentsOf: URL(fileURLWithPath: path)), data.count > 0 else {
            print("[Swifty] Unable to mock response from file: \(withFile).\(ofType): Make sure the filename and extension are correct, and the file is present in the bundle - \(bundle.bundleIdentifier ?? "") . Also make sure the file is not empty.")
            return self
        }
